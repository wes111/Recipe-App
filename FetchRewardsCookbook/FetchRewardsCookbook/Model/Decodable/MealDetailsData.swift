...
struct MealDetails: Decodable {
    ...
    func combineIngredients() -> [String] {
        ...
        return removeBadData(from: ingredientsArray)
    }
    
    func combineMeasurements() -> [String] {
        let measurementsArray = [strMeasure1, strMeasure2, strMeasure3,
        ...
        return removeBadData(from: measurementsArray)
    }
    
    func removeBadData(from dataArray: [String?]) -> [String] {
        // Unwrap all optionals and remove nils.
        let safeArray = dataArray.compactMap { $0 }
        // Remove empty Strings.
        let finalArray = safeArray.filter({ $0 != ""})
        return finalArray
    }
}
